...
\prod_{j=1}^m \sum_{i=1}^{n^{(c)}} \frac{\mu_i^{(c)}}{\mu^{(c)}} \underbrace{\int_{[0,1]^d} p_v\left(\widetilde{y}^{(j)}-\widetilde{\kappa}_i^{(c)}(\beta)\right)\text{ d}\beta}_{\mathcal{O}(N_i^{(c)})}}_{\mathcal{O}(m n^{(c)} \bar{N}^{(c)})}
\end{equation*}
\pause
