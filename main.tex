...
\begin{frame}[noframenumbering, plain]
	...
	\vspace{0.2cm}
		\begin{tabular}{@{}ll@{}}
		  ...
		  \textbf{Conclusions}    & summary and future research directions                           
		\end{tabular}
\end{frame}
...
\newline Why not perform classification directly over point clouds?
\end{itemize}
\end{frame}
...
\newline
\pause
Accordingly, we define the \textit{object shape} $\widetilde{\mathcal{S}}$ as a closed and non self-intersecting 
polygon (contour or surface) contained in the unit square $[-0.5,+0.5]^2$. 
\newline Such polygon is defined by a \textbf{shape vector} $\widetilde{S}$ stacking vertex coordinates.
\end{frame}
...
\begin{frame}{Tracking for general objects}
	Since the shape vectors $\{\widetilde{S}^{(c)}\}_{c=1}^C$ are referred to the unit square $[-0.5,+0.5]^2$, 
...
\begin{equation*}\begin{aligned}
\pi_{k|k}^{c} &\propto \exp\left(-J_k\right)\,\pi_{k|k-1}^{c} \\
J_k &\triangleq -\frac{1}{\tau_{\textrm{c}}} \,\log \mathcal{L}_k
\end{aligned}
\end{equation*}
for a suitable \textit{temperature} parameter $\tau_{\textrm{c}}>0$ and a suitable PC2S likelihood matrix $\mathcal{L}$.
\end{frame}
...
\begin{itemize}
	\item if $\eta\to 0$, we recover the standard MAP estimator (no smoothing);
	\item if $\eta\to +\infty$, we recover a zero-order hold estimator (no change).
\end{itemize}
\end{frame}
...
\begin{frame}{Tracking for general objects}
	...
	\begin{itemize}
		\item \textbf{dissimilarity matrix}
		\begin{equation*}
			[\mathcal{D}]_{ij}\propto \exp\left[-\frac{1}{\tau_{\textrm{p}}}\textrm{dissim}\left(\widetilde{S}^{(i)},\widetilde{S}^{(j)}\right) \right]
		\end{equation*}
		where $\tau_{\textrm{p}}>0$ is a temperature parameter.\newline
		This term makes the classifier robust against geometric ambiguities between similar shapes. 
		Dissimilarity metrics: Hausdorff, chamfer, earth mover, etc...
...
\newline
Assuming $\widetilde{\mathcal{Y}}$ is as an \textbf{Independent and Identically Distributed Cluster (IIDC)} Random Finite Set,
\begin{equation*}
...
\begin{frame}{Tracking for general objects}
To define, $\mathcal{L}^{\textrm{C}}\left(|\widetilde{\mathcal{Y}}|\,\big|\,\widetilde{S}^{(c)}\right)$ 
and $\mathcal{L}^{\textrm{S}}\left(\widetilde{y}\,\big|\widetilde{S}^{(c)}\right)$, we need to introduce:
\begin{itemize}
	\item \textbf{shape patch}: the object shape is decomposed into $n$ non-overlapping patches 
	\begin{equation*}
	...
	\pause
	\item \textbf{patch measure}: to each patch $\widetilde{\mathcal{S}}_i$ we associate a measure 
	\begin{equation*}
	\mu_i \triangleq \mu\left(\widetilde{\mathcal{S}}_i\right)
	\end{equation*}
	If $\widetilde{\mathcal{S}}$ is the polygon contour, $\mu_i$ is the \textbf{length} of the $i$-th polygon edge.
	\newline
	If $\widetilde{\mathcal{S}}$ is the polygon surface, $\mu_i$ is the \textbf{area} of the $i$-th polygon triangle.
\end{itemize}
\end{frame}
...
\begin{frame}{Tracking for general objects}
\textbf{Cardinality likelihood}
\begin{equation*}\begin{aligned}
\mathcal{L}^{\textrm{C}}\left(|\widetilde{\mathcal{Y}}|\,\big|\,\widetilde{S}^{(c)}\right) &\triangleq
\textrm{Poisson}\left(|\widetilde{\mathcal{Y}}|;\,\lambda^{(c)}\right)\\
\lambda^{(c)} &\triangleq \rho\,a^{(c)}\,b^{(c)}\,\sum_{i=1}^{n^{(c)}} \mu_i^{(c)}
\end{aligned}
\end{equation*}
where:
\begin{itemize}
	\item $\rho>0$ is the \textbf{sensor resolution} (expected number of points per unit measure);
	\item $a^{(c)},b^{(c)}$ are the object length and width (from shape library).
\end{itemize}
\textbf{IDEA}: "bigger" is the shape, larger is the number of points we expect in the cloud.
\end{frame}

\begin{frame}{Tracking for general objects}
\textbf{Spatial likelihood}
\newline
assuming that the point cloud is uniformly distributed over the object shape $\mathcal{S}$,
\begin{equation*}\begin{aligned}
\mathcal{L}^{\textrm{S}}\left(\widetilde{y}\,\big|\,\widetilde{S}^{(c)}\right) &\triangleq
\sum_{i=1}^{n^{(c)}} \frac{\mu_i^{(c)}}{\mu^{(c)}} \int_{[0,1]^d} \pi_v\left(\widetilde{y}-\widetilde{\kappa}_i^{(c)}(\beta)\right)\text{ d}\beta\\
\mu^{(c)} &\triangleq \sum_{i=1}^{n^{(c)}} \mu_i^{(c)}
\end{aligned}
\end{equation*}
where $d=1$ in the contour case, $d=2$ in the surface case, and:
\begin{itemize}
	\item $\pi_v(\cdot)$ is the sensor noise density;
	\item $\widetilde{\kappa}_i^{(c)}(\cdot)$ is the \textbf{patch covering kernel}. 
\end{itemize}
\pause
...
\begin{frame}{Tracking for general objects}
T2S is \textbf{linear} in the shape complexity $n^{(c)}$:
\begin{equation*}
\mathcal{L}\left(\widetilde{\mathcal{Y}}\,\big|\,\widetilde{S}^{(c)}\right)=
\underbrace{\underbrace{\textrm{Poisson}\left(|\widetilde{\mathcal{Y}}|;\,\lambda^{(c)}\right)}_{\mathcal{O}(1)}\,
\prod_{j=1}^m \sum_{i=1}^{n^{(c)}} \frac{\mu_i^{(c)}}{\mu^{(c)}} \underbrace{\int_{[0,1]^d} \pi_v\left(\widetilde{y}^{(j)}-\widetilde{\kappa}_i^{(c)}(\beta)\right)\text{ d}\beta}_{\mathcal{O}(N_i^{(c)})}}_{\mathcal{O}(m n^{(c)} \bar{N}^{(c)})}
\end{equation*}
\pause
The overall complexity is $\mathcal{O}(C m \bar{n} \bar{N})$:
\begin{itemize}
	\item $C$ is the number of shapes in the shape library;
	\item $m$ is the number of points in the cloud;
	\item $\bar{n}$ is the average number of patches across the shape library;
	\item $\bar{N}$ is the average number of MC particles across the shape library.
\end{itemize}
\end{frame}
